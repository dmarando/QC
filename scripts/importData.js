...
const transformAmmoRow = (row) => {
  ...
  if (make && model) {
    return {
      ...
      name: `${make.trim()} ${model.trim()} ${gauge ? gauge.trim() + ' ' : ''}${shotSize ? shotSize.trim() + ' ' : ''}${shotWeight ? shotWeight.trim() + 'oz' : ''}`.trim()
    };
  }
  return null;
};

// NEW: Row transformation function for events CSV
const transformEventRow = (row) => {
  const name = row['Name']; // Exact header is 'Name'
  if (name) {
    return {
      name: name.trim()
    };
